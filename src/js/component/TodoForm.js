  ...
  return (
    <>
      ...
      <form onSubmit={onSubmit}>
        <input placeholder="Add a todo" className="todo-input" type="text" value={value} onChange={handleValueChange} />
      </form>
      {todos.map((todo) => (
        <div
          key={todo.label}
        ...
        >
          <h5>{todo.label}</h5>
          <button className="todo-button" type="submit" onClick={() => handleDone(todo.label)}> Delete </button>
        </div>
      ))}
    </>
  );
